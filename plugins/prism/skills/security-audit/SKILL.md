...
- **The audited app itself integrates LLMs** (calls an LLM API, defines tools/agents, renders
  ...
  excessive agent tool permissions, per OWASP LLM Top 10).
...
Report findings as **high-confidence leads a human must confirm**. Even dedicated red-team
agents resolve only ~13% of real CVEs end-to-end (CVE-Bench). Confidence means a careful
reviewer would flag it, without any claim that it is weaponized.
