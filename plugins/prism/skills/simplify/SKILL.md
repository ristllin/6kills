...
- **Match the codebase.** Follow existing conventions (module system, error handling, naming,
  formatting); read neighboring files first. Simpler means *consistent* with the codebase's style.

## Balance (don't over-simplify)

Keep helpful abstractions, meaningful names, and necessary edge-case handling. The goal is
**lower cognitive load**, even at the cost of more lines. If a "simplification" makes the code harder to
